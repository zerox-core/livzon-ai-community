...
- 本子系统为**新增**，不改既有 `artifacts` / `activities` 报名上传 / `artifacts` 信件上传逻辑（后续竖线接入）。
- 消费方统一引用本子系统的 `storage_url`（或 asset id）；各自的 JSONB/列结构不变。
- `runner`：请先 `node server/sql/run_migrate.js` 建 `assets` 表，再重启 `node start.mjs`。
